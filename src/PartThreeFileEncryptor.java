 ...
 * @author Erik Costlow
 */
public class PartThreeFileEncryptor {
     ...
     */
    public static void encrypt(String inputFile, String outputFile, Path tempDirectory, String password)
        ...
        try {
            if (args.length >= 1) {
                if (args[0].equals(encryption)) {
                    ...
                    outFile = args[3];
                    password = args[1];
                    encrypt(inFile, outFile, tempDir, password);
        ...
        try {
            if (args.length >= 1) {
                if (args[0].equals(decryption)) {
                    ...
                    outFile = args[3];
                    password = args[1];
                    decrypt(tempDir, inFile, outFile, password);
                    password = null; //Wiping password after use
                }
            }
